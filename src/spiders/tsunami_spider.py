...
from src.items.tsunami import Tsunami, COLUMN_NAMES
...
# Use this when develop in order to not overcharge the server
...
class TsunamiSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        with open('datasets/{}.csv'.format(datetime.today().strftime("%y-%m-%d_%H%M")), 'w') as csv_file:
            writer = csv.DictWriter(csv_file, fieldnames=COLUMN_NAMES)
            writer.writeheader()
            for data in tsunamis:
